...
contract OptimismIntegrationBase is Test {
  ...
  uint256 constant POSITION_AMOUNT1 = 2000e18; // Amount of token1 for positions (must be above
  // minimum liquidity in SuperDCAListing)
  ...
  function setUp() public virtual {
    ...
    // Optionally pin a block by exporting OPTIMISM_BLOCK_NUMBER; otherwise use a stable recent
    // block
  ...
  function _fundTestAddresses() internal {
    // Fund test addresses with DCA tokens and WETH
    deal(DCA_TOKEN, user1, 10_000e18);
    deal(DCA_TOKEN, user2, 10_000e18);
    deal(DCA_TOKEN, address(this), 10_000e18);
  ...
  function _createTestPoolFromKey(PoolKey memory key, uint160 sqrtPriceX96)
    internal
  ...
  function _createFullRangePosition(
    PoolKey memory key,
    uint256 amount0,
    uint256 amount1,
    address recipient
    ...
    TokenAddresses memory tokens = TokenAddresses({
      token0: Currency.unwrap(key.currency0), token1: Currency.unwrap(key.currency1)
    });
  ...
  function _prepareTokens(TokenAddresses memory tokens, uint256 amount0, uint256 amount1) internal {
    ...
    IAllowanceTransfer(PERMIT2)
      .approve(tokens.token0, POSITION_MANAGER_V4, type(uint160).max, type(uint48).max);
    IAllowanceTransfer(PERMIT2)
      .approve(tokens.token1, POSITION_MANAGER_V4, type(uint160).max, type(uint48).max);
  }
  ...
  function _executePositionMint(
    ...
    // Look for Transfer event: Transfer(address indexed from, address indexed to, uint256 indexed
    // tokenId) Event signature: keccak256("Transfer(address,address,uint256)")
    bytes32 transferEventSignature =
  ...
  function _getCurrentPrice() internal pure returns (uint160) {
    ...
    return 2_505_414_483_750_479_311_864_138_015_696_896; // approximate value
  }
    ...
    returns (SwapResult memory result)
  {
    ...
    uint24 feeApplied;
    if (gauge.isInternalAddress(swapper)) feeApplied = gauge.internalFee();
    else if (swapper == _getKeeperAddress()) feeApplied = gauge.keeperFee();
    else feeApplied = gauge.externalFee();
      ...
      for (uint256 j = i + 1; j < results.length; j++) {
        if (results[i].amountIn == results[j].amountIn) {
          if (results[i].feeApplied > results[j].feeApplied) {
            assertLt(
              results[i].amountOut, results[j].amountOut, "Higher fee should result in lower output"
            );
          }
        }
      }
