...
contract OptimismStakingIntegration is OptimismIntegrationBase {
  ...
  function _setupListedToken(address token, uint256 amount0, uint256 amount1)
    internal
  ...
  function testFork_Stake_Success() public {
    ...
    assertEq(
      ...
      balanceBefore.userBalance - stakeAmount,
  ...
  function testFork_PreviewPending_AccurateCalculation() public {
    ...
    assertApproxEqAbs(
      pendingAfter, expectedReward, 1000, "Pending should match expected calculation"
    );
  }
  ...
  function testFork_RevertIf_SetMintRateCalledByGauge() public {
    ...
    vm.prank(address(gauge));
    vm.expectRevert(
      abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, address(gauge))
    );
    staking.setMintRate(newMintRate);
  }
