...
contract OptimismGaugeIntegration is OptimismIntegrationBase {
  ...
  function testFork_InitializePool_MustIncludeDCAToken() public {
    ...
    // ---- Act & Assert ----
    /// @dev Manually verified in the stack trace that the rever reason is
    // "PoolMustIncludeSuperDCAToken()" / Unsure how to use WrappedError here to properly test this
    // revert.
    vm.expectRevert();
  ...
  function testFork_BecomeKeeper_KingOfTheHill() public {
    ...
    deal(DCA_TOKEN, keeper1, 10_000e18);
    deal(DCA_TOKEN, keeper2, 10_000e18);
    ...
    assertGe(
      IERC20(DCA_TOKEN).balanceOf(keeper1),
      10_000e18 - 1000e18 + 1000e18,
      "Keeper1 should be refunded"
    );
  }
  ...
  function testFork_CompleteWorkflow_ListStakeReward() public {
    ...
    // In a successful scenario, pending rewards should be reduced and developer should receive
    // tokens The exact amounts depend on minting success and pool liquidity
  ...
  function testFork_DynamicFees_ActualSwapExecution() public {
    ...
    _prepareSwapTokens(
      Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), 0, swapAmount, keeperUser
    );
    _prepareSwapTokens(
    ...
    SwapResult memory externalResult =
    ...
    assertLt(
      internalResult.feeApplied,
      keeperResult.feeApplied,
      "Internal fee should be less than keeper fee"
    );
    assertLt(
      keeperResult.feeApplied,
      externalResult.feeApplied,
      "Keeper fee should be less than external fee"
    );

    // For same input amounts, lower fees should yield higher outputs
    assertTrue(
      internalResult.amountIn == keeperResult.amountIn, "Same input amounts required for comparison"
    );
    assertTrue(
      keeperResult.amountIn == externalResult.amountIn, "Same input amounts required for comparison"
    );

    assertGt(
      internalResult.amountOut, keeperResult.amountOut, "Internal user should receive more output"
    );
    assertGt(
      keeperResult.amountOut, externalResult.amountOut, "Keeper should receive more than external"
    );
  ...
  function testFork_DynamicFees_FeeUpdatesReflectedInSwaps() public {
    ...
    _prepareSwapTokens(
      Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), 0, swapAmount, testUser
    );
    ...
    SwapResult memory updatedInternalResult =
      _executeV4Swap(key, uint128(swapAmount), 0, testUser, false);
    _assertCorrectFeeApplied(updatedInternalResult, newInternalFee);

    // Verify the fee change affected the output
    assertNotEq(
      internalResult.amountOut, updatedInternalResult.amountOut, "Fee change should affect output"
    );
  }
  ...
  function testFork_DynamicFees_KeeperTransitions() public {
    ...
    _prepareSwapTokens(
      Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), 0, swapAmount, keeper1
    );
    ...
    _prepareSwapTokens(
      Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), 0, swapAmount, keeper2
    );
    ...
    assertLt(
      exKeeper1Result.amountOut,
      keeper1Result.amountOut,
      "Ex-keeper should get less than when they were keeper"
    );
    assertEq(
      keeper2Result.feeApplied, keeper1Result.feeApplied, "Both keepers should get same fee rate"
    );
  }
  ...
  function testFork_DynamicFees_EdgeCases() public {
    ...
    _prepareSwapTokens(
      Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), 0, swapAmount, edgeUser
    );
    ...
    assertGt(
      zeroFeeResult.amountOut,
      maxFeeResult.amountOut,
      "Zero fee should produce more output than max fee"
    );
  }
  ...
  function testFork_DynamicFees_BidirectionalSwaps() public {
    ...
    // Test DCA -> WETH
    _prepareSwapTokens(
      Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), swapAmount, 0, testUser
    );
    ...
    assertEq(
      dcaToWethResult.feeApplied,
      wethToDcaResult.feeApplied,
      "Bidirectional swaps should have same fee"
    );
  ...
  function testFork_DynamicFees_ConsecutiveSwaps() public {
    ...
    for (uint256 i = 1; i < numSwaps; i++) {
      assertEq(
        internalResults[i].feeApplied,
        internalResults[0].feeApplied,
        "Internal fee should be consistent"
      );
      assertEq(
        externalResults[i].feeApplied,
        externalResults[0].feeApplied,
        "External fee should be consistent"
      );
    }
  }
  ...
  function testFork_DynamicFees_ComprehensiveComparison() public {
    ...
    deal(DCA_TOKEN, keeperUser, 10_000e18);
    vm.startPrank(keeperUser);
    IERC20(DCA_TOKEN).approve(address(gauge), 5000e18);
    gauge.becomeKeeper(5000e18);
    vm.stopPrank();
    ...
    for (uint256 i = 0; i < users.length; i++) {
      _prepareSwapTokens(
        Currency.unwrap(key.currency0), Currency.unwrap(key.currency1), 0, swapAmount, users[i]
      );
    }
    ...
    for (uint256 i = 0; i < results.length; i++) {
      assertGt(
        results[i].amountOut,
        0,
        string(abi.encodePacked("Swap ", vm.toString(i), " should succeed"))
      );
      assertEq(
        results[i].amountIn,
        swapAmount,
        string(abi.encodePacked("Swap ", vm.toString(i), " input amount"))
      );
    }
  }
  ...
  function testFork_RevertWhen_UnverifiedRouterAttemptsSwap() public {
    ...
    // ---- Act & Assert ----
    // Attempt swap through unverified Universal Router - should revert with
    // SuperDCAGauge__UnauthorizedRouter
  ...
  function testFork_InternalSwap_DoesNotTriggerDistribution() public {
    ...
    assertEq(
      staking.lastMinted(),
      initialLastMinted,
      "Last minted time should not change for internal swap"
    );
  ...
  function testFork_KeeperSwap_DoesNotTriggerDistribution() public {
    ...
    // Verify lastMinted was NOT updated
    assertEq(
      staking.lastMinted(), initialLastMinted, "Last minted time should not change for keeper swap"
    );
  ...
  function testFork_ExternalSwap_WithNoRewards() public {
    ...
    assertEq(
      IERC20(DCA_TOKEN).balanceOf(deployer),
      initialDevBalance,
      "No rewards should be distributed with no stake"
    );
  }
  ...
  function testFork_ExternalSwap_WhenMintFails() public {
    ...
    assertEq(
      IERC20(DCA_TOKEN).balanceOf(deployer),
      initialDevBalance,
      "No tokens should be minted when mint fails"
    );
  ...
  function testFork_ExternalSwap_MultiplePoolDistribution() public {
    ...
    uint256 swapAmount = 1e18;
    _prepareSwapTokens(
      Currency.unwrap(wethKey.currency0),
      Currency.unwrap(wethKey.currency1),
      0,
      swapAmount,
      externalUser
    );
    ...
    uint256 expectedCommunityShare = wethPoolShare - expectedDevShare; // Community gets remaining
    // 50%
