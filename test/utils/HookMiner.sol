...
library HookMiner {
  ...
  /// @param deployer The address that will deploy the hook. In `forge test`, this will be the test
  /// contract `address(this)` or the pranking address In `forge script`, this should be
  /// `0x4e59b44847b379578588920cA78FbF26c0B4956C` (CREATE2 Deployer Proxy)
  /// @param flags The desired flags for the hook address
  /// @param creationCode The creation code of a hook contract. Example:
  /// `type(Counter).creationCode` @param constructorArgs The encoded constructor arguments of a
  /// hook contract. Example: `abi.encode(address(manager))`
  /// @return hookAddress salt and corresponding address that was found. The salt can be used in
  /// `new Hook{salt: salt}(<constructor arguments>)`
  function find(
    address deployer,
    uint160 flags,
