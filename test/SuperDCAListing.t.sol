...
import {
  LiquidityAmounts as PeripheryLiquidityAmounts
} from "lib/v4-periphery/src/libraries/LiquidityAmounts.sol";
...
contract SuperDCAListingTest is Test, Deployers {
  ...
  function setUp() public virtual {
    ...
    listing = new SuperDCAListing(
      address(dcaToken), manager, positionManagerV4, developer, IHooks(address(0))
    );

    // Build a pool key with dynamic fee and assign hooks after computing flags deployment address
    // for gauge-style
    ...
    returns (uint256 liq)
  {
    ...
    liq = PeripheryLiquidityAmounts.getLiquidityForAmounts(
      sqrtPriceX96, sqrtA, sqrtB, amount0, amount1
    );
  }
    ...
    returns (uint256 nfpId)
  {
    ...
    planner = planner.add(
      ...
      abi.encode(
        _key, lower, upper, liquidity, type(uint128).max, type(uint128).max, owner, bytes("")
      )
    );
  ...
  function _mintNarrow(
    ...
    uint256 amount1,
    address owner
  ) internal returns (uint256 nfpId) {
    _fundAndApprove(owner, Currency.unwrap(_key.currency0), amount0);
    _fundAndApprove(owner, Currency.unwrap(_key.currency1), amount1);
    ...
    uint256 liquidity = PeripheryLiquidityAmounts.getLiquidityForAmounts(
      sqrtPriceX96, sqrtA, sqrtB, amount0, amount1
    );
  ...
  function _mintFullRangeWithNativeETH(
    PoolKey memory _key,
    uint256 amount0,
    uint256 amount1,
    address owner
    ...
    uint256 ethValue = 0;
    if (_key.currency0.isAddressZero()) ethValue = amount0;
    else if (_key.currency1.isAddressZero()) ethValue = amount1;

    vm.prank(owner);
    positionManagerV4.modifyLiquidities{value: ethValue}(calls, block.timestamp);
  }

  function _accrueFeesByDonationWithNativeETH(PoolKey memory _key, uint256 amt0, uint256 amt1)
    internal
  {
    address t0 = Currency.unwrap(_key.currency0);
    address t1 = Currency.unwrap(_key.currency1);
    ...
    // Calculate ETH value to send
    uint256 ethValue = 0;
    if (_key.currency0.isAddressZero()) ethValue = amt0;
    else if (_key.currency1.isAddressZero()) ethValue = amt1;
...
contract Constructor is SuperDCAListingTest {
  ...
  function test_RevertWhen_InvalidAdmin() public {
    ...
    new SuperDCAListing(
      address(dcaToken), manager, positionManagerV4, address(0), IHooks(address(0))
    );
  }
}
...
contract SetMinimumLiquidity is SuperDCAListingTest {
  ...
  function test_RevertWhen_SetMinimumLiquidityCalledByNonAdmin(address _notAdmin, uint256 _newMin)
    public
  {
    vm.assume(_notAdmin != developer && _notAdmin != address(0));
    vm.prank(_notAdmin);
...
contract List is SuperDCAListingTest {
  ...
  function test_EmitsTokenListedAndRegistersToken_When_ValidFullRangeAndLiquidity() public {
    ...
    PoolKey memory keyWithDca =
      _createPoolKey(address(dcaToken), address(alt), LPFeeLibrary.DYNAMIC_FEE_FLAG);
    keyWithDca = _initPoolWithHook(keyWithDca, key.hooks);

    // Mint a full-range NFP
    uint256 nfpId = _mintFullRange(keyWithDca, 2000e18, 2000e18, address(this));
  ...
  function test_RevertWhen_IncorrectHookAddress() public {
    ...
    uint256 nfpId = _mintFullRange(wrongHookKey, 1000e18, 1000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), nfpId);
  ...
  function test_RevertWhen_PositionIsNotFullRange() public {
    ...
    uint256 nfpId =
      _mintNarrow(key, minTick + key.tickSpacing, maxTick, 1000e18, 1000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), nfpId);
  ...
  function test_RevertWhen_PartialRange_UpperWrong() public {
    ...
    int24 maxTick = TickMath.maxUsableTick(key.tickSpacing);
    uint256 nfpId =
      _mintNarrow(key, minTick, maxTick - key.tickSpacing, 1000e18, 1000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), nfpId);
    vm.expectRevert(SuperDCAListing.SuperDCAListing__NotFullRangePosition.selector);
    listing.list(nfpId);
  }
  ...
  function test_RevertWhen_TokenAlreadyListed() public {
    uint256 id1 = _mintFullRange(key, 2000e18, 2000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), id1);
    listing.list(id1);

    uint256 id2 = _mintFullRange(key, 2000e18, 2000e18, address(this));
  ...
  function test_RevertWhen_CallerIsNotOwner() public {
    ...
    uint256 nfpId = _mintFullRange(key, 2000e18, 2000e18, unauthorizedCaller);
  ...
  function test_RegistersTokenAndTransfersNfp_When_DcaTokenIsCurrency0() public {
    ...
    uint256 nfpId = _mintFullRange(keyWithDca0, 2000e18, 2000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), nfpId);
  ...
  function test_RegistersTokenAndTransfersNfp_When_DcaTokenIsCurrency1() public {
    ...
    uint256 nfpId = _mintFullRange(keyWithDca1, 2000e18, 2000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), nfpId);
...
contract CollectFees is SuperDCAListingTest {
  ...
  function test_CollectFees_IncreasesRecipientBalances_When_CalledByAdmin() public {
    // Mint and list
    uint256 nfpId = _mintFullRange(key, 2000e18, 2000e18, address(this));
    IERC721(address(positionManagerV4)).approve(address(listing), nfpId);
    listing.list(nfpId);
  ...
  function test_CollectFees_WorksWithNativeETHAsToken0() public {
    ...
    uint256 nfpId = _mintFullRangeWithNativeETH(nativeKey, 2000e18, 2000e18, owner);
    ...
    assertGt(
      dcaToken.balanceOf(recipient), tokenBalanceBefore, "DCA token fees should be collected"
    );
  }
