...
contract SetMintRate is SuperDCAStakingTest {
  ...
  function test_RewardsUseCorrectRateForEachTimePeriod() public {
    ...
    assertEq(
      accrued,
      expectedTotal,
      "Total rewards should use old rate for period 1 and new rate for period 2"
    );
  }
}
...
contract Stake is SuperDCAStakingTest {
  ...
  function test_FirstStakerCannotHarvestBankedTime() public {
    ...
    assertEq(
      staking.lastMinted(), deployTime + emptyPeriod, "lastMinted should be updated to current time"
    );
...
contract TokenRewardInfos is SuperDCAStakingTest {
  ...
  function test_MultipleStakeUnstakeBeforeAccrue_PreservesRewards() public {
    ...
    // Period 2: 150e18 staked, 50 seconds = 5000 rewards minted -> index increases by (5000 * 1e18
    // / 150e18) = 33 150e18 * 33 / 1e18 = 4950 rewards
