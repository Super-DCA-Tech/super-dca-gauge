...
contract SuperDCAGaugeTest is Test, Deployers {
  ...
  IAllowanceTransfer public constant PERMIT2 =
    IAllowanceTransfer(0x000000000022D473030F116dDEE9F6B43aC78BA3); // Real Permit2 address
  ...
  function _createPoolKey(address tokenA, address tokenB, uint24 fee)
    internal
    view
  ...
  function setUp() public virtual {
    ...
    //PLEASE CHECK THIS DOWN FOR ME !!!!! IT ASK FOR THE CALLER OF collectProtocolFees function MUST
    // BE THE ProtocolFeeController the role is granted by owner calling  setProtocolFeeController()
    // function in IProtocolFees contract ,
    ...
    vm.mockCall(
      address(hook), abi.encodeWithSelector(IS_TOKEN_LISTED, address(weth)), abi.encode(true)
    );
...
contract ConstructorTest is SuperDCAGaugeTest {
  function test_initialization() public view {
    ...
    assertEq(
      hook.internalFee(), hook.INTERNAL_POOL_FEE(), "Internal fee should be initialized to constant"
    );
    assertEq(
      hook.externalFee(), hook.EXTERNAL_POOL_FEE(), "External fee should be initialized to constant"
    );
    assertEq(
      hook.keeperFee(), hook.KEEPER_POOL_FEE(), "Keeper fee should be initialized to constant"
    );
    ...
    assertFalse(
      permissions.afterAddLiquidityReturnDelta, "afterAddLiquidityReturnDelta should be disabled"
    );
    assertFalse(
      permissions.afterRemoveLiquidityReturnDelta,
      "afterRemoveLiquidityReturnDelta should be disabled"
    );
...
contract BeforeInitializeTest is SuperDCAGaugeTest {
  function test_beforeInitialize() public {
    ...
    PoolKey memory correctKey =
      _createPoolKey(address(0xBEEF), address(dcaToken), LPFeeLibrary.DYNAMIC_FEE_FLAG);
  ...
  function test_beforeInitialize_revert_wrongToken() public {
    ...
    PoolKey memory wrongTokenKey =
      _createPoolKey(address(weth), address(0xBEEF), LPFeeLibrary.DYNAMIC_FEE_FLAG);
...
contract BeforeAddLiquidityTest is SuperDCAGaugeTest {
  function test_distribution_on_addLiquidity() public {
    ...
    if (mintAmount % 2 == 1) communityShare += 1;

    // Verify distributions
    assertEq(
      dcaToken.balanceOf(developer), developerShare, "Developer should receive correct share"
    );
  ...
  function test_whenMintFails_onAddLiquidity() public {
    ...
    assertEq(
      dcaToken.balanceOf(developer), 0, "Developer balance should remain zero when mint fails"
    );

    // lastMinted should still update
    assertEq(
      staking.lastMinted(), startTime + elapsed, "lastMinted should update even when minting fails"
    );
  }
}
...
contract BeforeRemoveLiquidityTest is SuperDCAGaugeTest {
  function test_distribution_on_removeLiquidity() public {
    ...
    // They are split evenly unless the mintAmount is odd, in which case the community gets 1 extra
    // wei.
    uint256 mintAmount = elapsed * mintRate; // 20 * 100 = 2000
  ...
  function test_whenMintFails_onRemoveLiquidity() public {
    ...
    assertEq(
      dcaToken.balanceOf(developer),
      devBalanceBefore,
      "Developer balance should be unchanged after failed mint"
    );
...
contract RewardsTest is SuperDCAGaugeTest {
  ...
  function setUp() public override {
    ...
    bytes4 IS_TOKEN_LISTED = bytes4(keccak256("isTokenListed(address)"));
    vm.mockCall(
      address(hook), abi.encodeWithSelector(IS_TOKEN_LISTED, address(usdc)), abi.encode(true)
    );
  ...
  function test_getPendingRewards() public {
    ...
    assertEq(
      staking.previewPending(address(weth)),
      expectedRewards,
  ...
  function test_multiple_pool_rewards() public {
    ...
    assertEq(
      staking.getUserStake(address(this), address(weth)), 100e18, "WETH stake amount incorrect"
    );
    assertEq(
      staking.getUserStake(address(this), address(usdc)), 300e18, "USDC stake amount incorrect"
    );
...
contract AccessControlTest is SuperDCAGaugeTest {
  ...
  function test_Should_AllowAdminToUpdateManager() public {
    ...
    assertFalse(
      hook.hasRole(hook.MANAGER_ROLE(), newManagerUser),
      "New manager should not have role initially"
    );
...
contract SetFeeTest is AccessControlTest {
  ...
  function test_Should_AllowManagerToSetInternalFee() public {
    ...
    emit SuperDCAGauge.FeeUpdated(
      SuperDCAGauge.FeeType.INTERNAL, initialInternalFee, newInternalFee
    );
  ...
  function test_Should_AllowManagerToSetExternalFee() public {
    ...
    vm.expectEmit(true, true, true, true);
    emit SuperDCAGauge.FeeUpdated(
      SuperDCAGauge.FeeType.EXTERNAL, initialExternalFee, newExternalFee
    );
  ...
  function test_RevertWhen_NonManagerSetsInternalFee() public {
    vm.expectRevert(
      abi.encodeWithSelector(
        ACCESS_CONTROL_UNAUTHORIZED_ACCOUNT_SELECTOR, nonManagerUser, hook.MANAGER_ROLE()
      )
    );
    vm.prank(nonManagerUser);
    hook.setFee(SuperDCAGauge.FeeType.INTERNAL, newInternalFee);
...
contract SetVerifiedRouterTest is AccessControlTest {
  ...
  function test_RevertWhen_UnverifiedRouterAttemptsSwap() public {
    ...
    assertFalse(
      hook.verifiedRouters(address(swapRouter)), "Swap router should not be verified initially"
    );
    ...
    // Note: The error is wrapped in a Uniswap V4 WrappedError, so we cannot catch the specific
    // selector
    vm.expectRevert();
...
contract BecomeKeeperTest is SuperDCAGaugeTest {
  ...
  function test_becomeKeeper_replaceKeeper() public {
    ...
    assertEq(
      dcaToken.balanceOf(keeper2), 1000e18 - secondDeposit, "Second keeper balance should decrease"
    );
  ...
  function test_keeperFeeStructure() public {
    ...
    assertEq(
      hook.KEEPER_POOL_FEE(), 1000, "Keeper fee constant should be 0.10% (1000 basis points)"
    );
    assertEq(
      hook.EXTERNAL_POOL_FEE(), 5000, "External fee constant should be 0.50% (5000 basis points)"
    );
  ...
  function test_keeperStakingIndependentFromRewardStaking() public {
    ...
    assertEq(
      staking.getUserStake(keeper1, address(weth)), rewardStake, "Reward stake should be separate"
    );
    assertEq(
      staking.totalStakedAmount(), rewardStake, "Total staked should only include reward stakes"
    );

    uint256 expectedBalance = 1000e18 - keeperDeposit - rewardStake;
    assertEq(
      dcaToken.balanceOf(keeper1), expectedBalance, "Balance should account for both deposits"
    );
    ...
    assertEq(
      ...
      "Hook should hold only keeper deposit"
    );
  }
  ...
  function test_multipleKeeperChanges() public {
    ...
    assertEq(
      dcaToken.balanceOf(keeper1), 1000e18 - deposit3, "Keeper1 should have paid final deposit"
    );
  ...
  function test_keeperFeeUpdatesAffectSwaps() public {
    ...
    assertEq(
      hook.keeperFee(), hook.KEEPER_POOL_FEE(), "Initial keeper fee should be constant value"
    );
