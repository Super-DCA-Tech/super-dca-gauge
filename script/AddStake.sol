...
contract AddStake is Script {
  ...
  function setUp() public {
    ...
    if (stakingAddress == address(0)) revert("STAKING_ADDRESS environment variable not set.");
    tokenToStake = vm.envAddress("TOKEN_TO_STAKE");

    amountToStake = vm.envUint("AMOUNT_TO_STAKE");
    if (amountToStake == 0) revert("AMOUNT_TO_STAKE environment variable not set or is zero.");
  }
