...
contract AddStake is Script {
    ...
    function run() public {
        ...
        // Approve the gauge to spend our DCA tokens
        console2.log("Approving DCA token spend...");
        IERC20(DCA_TOKEN).approve(address(gauge), amountToStake);
        console2.log("Approved DCA token spend.");

        // Add stake to the specified token
        console2.log("Calling stake()...");
        gauge.stake(tokenToStake, amountToStake);
        console2.log("Called stake().");
