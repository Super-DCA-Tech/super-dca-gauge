...
contract SetFees is Script {
  ...
  function setUp() public {
    ...
    if (gaugeAddress == address(0)) revert("GAUGE_ADDRESS environment variable not set.");
    ...
    if (feeFromEnv > type(uint24).max) revert("NEW_FEE exceeds maximum value for uint24.");
  ...
  function run() public {
    ...
    if (feeType == SuperDCAGauge.FeeType.INTERNAL) feeTypeName = "INTERNAL";
    else if (feeType == SuperDCAGauge.FeeType.EXTERNAL) feeTypeName = "EXTERNAL";
    else feeTypeName = "KEEPER";
    console2.log("Setting fee type:", feeTypeName);
    console2.log("New fee value:", newFee);

    uint24 oldFee;
    if (feeType == SuperDCAGauge.FeeType.INTERNAL) oldFee = gauge.internalFee();
    else if (feeType == SuperDCAGauge.FeeType.EXTERNAL) oldFee = gauge.externalFee();
    else oldFee = gauge.keeperFee();
    ...
    uint24 currentFee;
    if (feeType == SuperDCAGauge.FeeType.INTERNAL) currentFee = gauge.internalFee();
    else if (feeType == SuperDCAGauge.FeeType.EXTERNAL) currentFee = gauge.externalFee();
    else currentFee = gauge.keeperFee();
    console2.log("Current fee value:", currentFee);

    if (currentFee == newFee) console2.log("Successfully set fee.");
    else console2.log("ERROR: Failed to set fee.");
