...
contract SuperDCAStaking is ISuperDCAStaking, Ownable2Step {
  ...
  function _accumulatePendingRewards(TokenRewardInfo storage info) internal {
    ...
    if (info.stakedAmount > 0) {
      ...
      if (delta > 0) info.pendingReward += Math.mulDiv(info.stakedAmount, delta, 1e18);
    }
  }
  ...
  function unstake(address token, uint256 amount) external override {
    ...
    if (userStakes[msg.sender][token] == 0) userTokenSet[msg.sender].remove(token);
  ...
  /**
   * @notice Calculates and returns the reward amount for a specific token bucket since last
   * accrual.
