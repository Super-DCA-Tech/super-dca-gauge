 ...
 * @notice Manages token listing for Super DCA by validating and taking custody of Uniswap V4 NFT
 * positions.
 * @dev This contract implements a token listing system where tokens become eligible for DCA
 * operations
...
contract SuperDCAListing is ISuperDCAListing, Ownable2Step {
  ...
  /**
   * @notice Lists a token for DCA operations by validating and taking custody of a Uniswap V4 NFT
   * position.
  ...
  function list(uint256 nftId) external override {
    ...
    if (address(key.hooks) != address(expectedHooks)) {
      revert SuperDCAListing__IncorrectHookAddress();
    }
    ...
    // This prevents gaming with partial liquidity ranges
    {
      ...
      (uint256 amount0, uint256 amount1) =
        _getAmountsForKey(key, _tickLower, _tickUpper, _liquidity);
